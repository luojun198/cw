...
const dialogTitle = computed(() => (dialogType.value === 'add' ? '新增功能分类' : '编辑功能分类'))
const form = ref<any>({ type: 'func_class', status: 'active' })
...
async function fetchData() {
  try {
    const r = await request.get<any[]>('/base/aux-items', { params: { type: 'func_class' } })
    list.value = r.data
  } catch (error) {
    showOperationError('查询功能分类列表', error)
  }
}

function openDialog(t: string, row?: any) {
  dialogType.value = t
  form.value = t === 'add' ? { type: 'func_class', status: 'active' } : { ...row }
...
async function handleSave() {
  try {
    if (dialogType.value === 'add') {
      ...
      showSuccess('功能分类新增成功')
    } else {
      await request.put(`/base/aux-items/${form.value.id}`, form.value)
      showSuccess('功能分类修改成功')
    }
    dialogVisible.value = false
    fetchData()
  } catch (error) {
    showOperationError(dialogType.value === 'add' ? '新增功能分类' : '修改功能分类', error)
  }
}

async function handleDelete(row: any) {
  const confirmed = await useDeleteConfirm(`功能分类「${row.code} ${row.name}」`)
  if (!confirmed) return
  ...
  try {
    ...
    showOperationError('删除功能分类', error)
  }
}
