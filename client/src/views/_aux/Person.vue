...
const form = ref<any>({ type: 'person', status: 'active' })
...
async function fetchData() {
  try {
    const r = await request.get<any[]>('/base/aux-items', { params: { type: 'person' } })
    list.value = r.data
  } catch (error) {
    showOperationError('查询人员列表', error)
  }
}

function openDialog(t: string, row?: any) {
  dialogType.value = t
  form.value = t === 'add' ? { type: 'person', status: 'active' } : { ...row }
...
async function handleSave() {
  try {
    ...
    showOperationError(dialogType.value === 'add' ? '新增人员' : '修改人员', error)
  }
}

async function handleDelete(row: any) {
  const confirmed = await useDeleteConfirm(`人员「${row.code} ${row.name}」`)
  if (!confirmed) return
