    ...
    <div class="page-header">
      ...
      <div>
        <el-input
          ...
          prefix-icon="Search"
        />
        <el-button type="primary" @click="openDialog('add')">新增单位</el-button>
      </div>
    </div>
...
const dialogTitle = computed(() => (dialogType.value === 'add' ? '新增往来单位' : '编辑往来单位'))
const form = ref<any>({ type: 'supplier', status: 'active' })
...
async function fetchData() {
  try {
    const r = await request.get<any[]>('/base/aux-items', { params: { type: 'supplier' } })
    list.value = r.data
  } catch (error) {
    showOperationError('查询往来单位列表', error)
  }
}

function openDialog(t: string, row?: any) {
  dialogType.value = t
  form.value = t === 'add' ? { type: 'supplier', status: 'active' } : { ...row }
...
async function handleSave() {
  try {
    if (dialogType.value === 'add') {
      ...
      showSuccess('往来单位新增成功')
    } else {
      await request.put(`/base/aux-items/${form.value.id}`, form.value)
      showSuccess('往来单位修改成功')
    }
    dialogVisible.value = false
    fetchData()
  } catch (error) {
    showOperationError(dialogType.value === 'add' ? '新增往来单位' : '修改往来单位', error)
  }
}

async function handleDelete(row: any) {
  const confirmed = await useDeleteConfirm(`往来单位「${row.code} ${row.name}」`)
  if (!confirmed) return
  ...
  try {
    ...
    showOperationError('删除往来单位', error)
  }
}
