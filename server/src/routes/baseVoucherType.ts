...
router.get('/voucher-types', (req: AuthRequest, res) => {
  ...
  const list = db
    .prepare(
      'SELECT * FROM voucher_types WHERE account_set_id=? OR account_set_id IS NULL ORDER BY sort_order'
    )
...
router.post('/voucher-types', operationLog('新增凭证类型', '基础设置'), (req: AuthRequest, res) => {
  const { name, code, prefix, description, sort_order } = req.body
  const db = getDb()
  const id = uuidv4()
  db.prepare(
    'INSERT INTO voucher_types (id, account_set_id, name, code, prefix, description, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run(id, req.accountSetId, name, code, prefix, description, sort_order || 0)
...
router.put(
  ...
  (req: AuthRequest, res) => {
    ...
    const db = getDb()
    db.prepare(
      'UPDATE voucher_types SET name=?, code=?, prefix=?, description=?, sort_order=? WHERE id=?'
    ).run(name, code, prefix, description, sort_order, id)
