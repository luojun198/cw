import { getDb, seedRoles } from '../db/index.ts'
import { v4 as uuidv4 } from 'uuid'
import bcrypt from 'bcryptjs'
import { readFileSync, existsSync } from 'fs'
import { join, resolve, dirname } from 'path'
import { fileURLToPath } from 'url'
import { importAcdTemplateToAccountSet } from './importAcdToCurrentAccountSet.ts'
...
// 凭证类型
const VOUCHER_TYPES = [
  { name: '记账凭证', code: 'JZ', prefix: 'JZ' },
  { name: '收款凭证', code: 'SK', prefix: 'SK' },
  { name: '付款凭证', code: 'FK', prefix: 'FK' },
  { name: '转账凭证', code: 'ZZ', prefix: 'ZZ' },
]
...
function insertVoucherTypes(accountSetId: string) {
  const db = getDb()
  const insert = db.prepare(`
    INSERT INTO voucher_types (id, account_set_id, name, code, prefix, sort_order)
    VALUES (?, ?, ?, ?, ?, ?)
  `)
  VOUCHER_TYPES.forEach((t, i) => {
    insert.run(uuidv4(), accountSetId, t.name, t.code, t.prefix, i)
  })
}

function createDefaultAdmin(accountSetId: string) {
  const db = getDb()
  const adminRole = db.prepare('SELECT * FROM roles WHERE code = ?').get('admin') as any
  if (!adminRole) return

  const password = bcrypt.hashSync('admin123', 10)
  db.prepare(
    `
    INSERT OR IGNORE INTO users (id, account_set_id, username, password, nickname, role_id, status)
    VALUES (?, ?, ?, ?, ?, ?, 'active')
  `
  ).run(uuidv4(), accountSetId, 'admin', password, '系统管理员', adminRole.id)
}
